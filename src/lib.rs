...
lazy_static! {
    #[derive(Debug)]
    pub static ref CRLF: Vec<u8> = "\r\n".as_bytes().to_vec();
    pub static ref DOUBLE_CRLF: Vec<u8> = "\r\n\r\n".as_bytes().to_vec();
}
    ...
    T: PartialEq,
{
    fn new(size: usize) -> Self {
        ArrayMap {
            key_array: Vec::with_capacity(size),
            value_array: Vec::with_capacity(size),
        }
    }

    fn get(&self, key: T) -> Option<&T> {
        let index = self.key_array.iter().position(|x| x == &key);
        match index {
            Some(i) => self.value_array.get(i),
            _ => None,
        }
    }
...
trait SelfCmp<T> {
    fn contains_part(&self, other: &Vec<T>) -> Option<(usize, usize)>;
}

impl SelfCmp<u8> for Vec<u8> {
    fn contains_part(&self, other: &Vec<u8>) -> Option<(usize, usize)> {
        ...
        let i = 0usize;
        loop {
            if i > self_len - other_len {
                break;
            }
            if &self[i..i + other_len - 1] == other {
                return Some((i, i + other_len - 1));
            }
        }
        None
    }
